...
<template>
	<v-card>
		<v-card-title class="pb-0">
			<v-icon small class="mr-1">opacity</v-icon>
				...
				<v-layout row wrap align-center>
					<v-flex v-if="currentTool && currentTool.extruders.length > 1" class="ma-1">
						...
						<v-btn-toggle v-model="mix" mandatory multiple>
							<v-btn flat value="mix" :disabled="uiFrozen" color="primary">{{ $t('panel.extrude.mix') }}</v-btn>
							<v-btn
								flat
					...
					<v-flex class="ma-1">
						<p class="mb-1">{{ $t('panel.extrude.amount', ['mm']) }}</p>
						<v-btn-toggle v-model="amount" mandatory>
							<v-btn
								flat
								v-for="(amount, index) in extruderAmounts"
								:key="index"
								:value="amount"
								:disabled="uiFrozen"
								color="primary"
					...
					<v-flex class="ma-1">
						<p class="mb-1">{{ $t('panel.extrude.feedrate', ['mm/s']) }}</p>
						<v-btn-toggle v-model="feedrate" mandatory>
							<v-btn
								flat
								v-for="(feedrate, index) in extruderFeedrates"
								:key="index"
								:value="feedrate"
								:disabled="uiFrozen"
								color="primary"
	...
	computed: {
		...
		canExtrude() {
			if (this.currentTool && this.currentTool.heaters.length) {
				...
				const heaters = this.heat.heaters,
					minTemp = this.heat.coldExtrudeTemperature;
		...
		canRetract() {
			if (this.currentTool && this.currentTool.heaters.length) {
				const selectedHeaters = this.mixValue[0] === 'mix' ? this.currentTool.heaters : this.mixValue;
				const heaters = this.heat.heaters,
					minTemp = this.heat.coldRetractTemperature;
				return !selectedHeaters.some(heater => heaters[heater].current < minTemp);
			}
			return false;
		},
		...
		async buttonClicked(extrude) {
			...
			if (this.mix.length === 1 && this.mix[0] === 'mix') {
				...
				amounts = this.currentTool.extruders.map(extruder =>
					this.mix.indexOf(extruder) !== -1 ? this.amount : 0
				);
			}
